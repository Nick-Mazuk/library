...
// heavily borrowed from https://github.com/simov/slugify/blob/master/slugify.js
export const slugify = (string: string): string => {
    ...
    return (
        string
            .split('')
            // eslint-disable-next-line unicorn/no-reduce -- should fix in the future
            .reduce((result, ch) => {
                return result + (charMap[ch] ?? ch).replace(/[^\w\s$*_+~.()'"!\-:@]+/gu, '')
            }, '')
            .trim()
            .replace(new RegExp(`[\\s${separator}]+`, 'gu'), separator)
            .replace(new RegExp(`[^a-zA-Z0-9${separator}]`, 'gu'), '')
            .replace(new RegExp(`[\\s${separator}]+`, 'gu'), separator)
            .toLowerCase()
    )
}
